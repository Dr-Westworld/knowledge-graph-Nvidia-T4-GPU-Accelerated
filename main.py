...
Extract from:"""
    ...
    def _enhance_connectivity(self, chunk: str, base_triples: List[Tuple[str, str, str, str, str, float]]) -> List[Tuple[str, str, str, str, str, float]]:
        ...
        # Add "related_to" for co-occurring entities
        for i, ent1 in enumerate(entity_list):
            for ent2 in entity_list[i+1:]:
                if ent1.lower() in chunk.lower() and ent2.lower() in chunk.lower():
                    # Check if direct connection exists
                    existing = any(
                        (src == ent1 and tgt == ent2) or (src == ent2 and tgt == ent1)
                        for src, _, _, tgt, _, _ in base_triples
                    )
...
class GPUTripleStore:
    ...
    def add_triples(self, triples: List[Tuple[str, str, str, str, str, float]]):
        ...
        if self.triples_df is None:
            ...
            self.triples_df = cudf.concat([self.triples_df, new_df], ignore_index=True)
...
class StreamableKGPipeline:
    ...
    def process_text(self, text: str) -> Iterator[Dict]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            chunk_triples = self.extractor.extract_with_context(
                chunk,
            ...
            if i > 0 and i % 10 == 0 and self.triple_store.triples_df is not None:
                gds_storage.save_checkpoint(self.triple_store.triples_df, f"batch_{i:04d}.parquet")
